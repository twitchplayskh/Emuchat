...
# Launch.py # Copyright (C) 2014 : Alex Edwards
...
def press(button, holdTime='1'):
    ...
    if button == '!wait':
        ...
        if (holdTime == '!hold' or holdTime == '!h') or (holdTime == '!release' or holdTime == '!r'):
            s.send(bytes(
                "PRIVMSG #%s :Hold/Release and Wait are not compatible holding for 1 second!\r\n" % (CHAT_CHANNEL)))
            holdTime = 1
        time.sleep(holdTime)
        button = None

    if str(holdTime).lower() == '!hold' or str(
            holdTime).lower() == '!h':  # Consider making !hold and !release NOT unpause the emulator when pausing is on if possible
        if button:
            win32api.keybd_event(VK_CODE[button], 0, 0, 0)
            HOLDING[button] = True
        ...
        for proc in psutil.process_iter():
            try:
                ...
                if pinfo['name'] == 'ePSXe.exe' and APP.lower() == 'epsxe':
                    ...
                    EMUPROCESSID = pinfo['pid']
                    EMUPROCESS = psutil.Process(EMUPROCESSID)
                elif pinfo['name'] == "nullDC.exe" and APP.lower() == 'nullDC':
                    with consoleLock:
                        print 'Found process nullDC.exe! PID: ' + str(pinfo['pid'])
                    EMUPROCESSID = pinfo['pid']
                    EMUPROCESS = psutil.Process(EMUPROCESSID)
    ...
    while True:
        if time.time() > last_command + democracy_time:
            ...
            alloutputs = {'Up': up_counter, 'Right': right_counter, 'Down': down_counter, 'Left': left_counter,
                          'Start': start_counter, 'Select': select_counter, 'B': b_counter, 'A': a_counter,
                          'S': s_counter, 'T': t_counter,
                          'LB': lb_counter, 'L2': lt_counter, 'L3': l3_counter, 'RB': rb_counter, 'R2': rt_counter,
                          'R3': r3_counter,
                          'Left Stick Up': lsu_counter, 'Left Stick Right': lsr_counter, 'Left Stick Down': lsd_counter,
                          'Left Stick Left': lsl_counter,
                          'Right Stick Up': rsu_counter, 'Right Stick Right': rsr_counter,
                          'Right Stick Down': rsd_counter, 'Right Stick Left': rsl_counter,
                          'Pausing On': pausing_counter, 'Pausing Off': unpausing_counter
                          }
            if (
                    up_counter + right_counter + down_counter + left_counter + start_counter + select_counter + b_counter + a_counter + s_counter + t_counter + b_counter + lt_counter + rb_counter + rt_counter + lsu_counter + lsr_counter + lsd_counter + lsl_counter + rsu_counter + rsr_counter + rsd_counter + rsl_counter + pausing_counter + unpausing_counter + l3_counter + r3_counter == 0):
                selected_c = "None"
            else:
...
while True:
    if os.path.isfile("settings.txt"):
        ...
        settings.append("; Currently Supported: vba, epsxe, and pcsx2 fightcade, nullDC,")
        with consoleLock:
            print("Name of the application you run the file from.")
            print("Currently Supported: vba, epsxe, pcsx2 fightcade, nullDC,")
...
if mode.lower() == "anarchy":
    ...
    while 1:
        ...
        for line in temp:
            ...
            # Take in output
            if (out.lower() == '!hold' or out.lower() == '!h') or (
                    out.lower() == '!release' or out.lower() == '!r') or out.lower() == 'all':
                tmp = repeatTimes
                repeatTimes = out
                out = str(tmp).lower()
